...
public record OpenQuestSelectionMessage(Component title, List<SelectableQuest> quests, int entityId, String questLineId) {
    ...
    public static OpenQuestSelectionMessage decode(FriendlyByteBuf buffer) {
        ...
        return new OpenQuestSelectionMessage(title, quests, id, buffer.readUtf());
    }

    public void handle(Supplier<NetworkEvent.Context> supplier) {
        if (this.entityId() != -1) ClientQuests.lastTalkedEntityId = this.entityId();
        Minecraft.getInstance().setScreen(new SelectQuestScreen(this.title(), this.quests(), ClientQuests.lastTalkedEntityId, this.questLineId));
    }
}
