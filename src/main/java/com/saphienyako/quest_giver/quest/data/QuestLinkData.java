package com.saphienyako.quest_giver.quest.data;

import com.google.gson.JsonElement;
import com.google.gson.JsonObject;
import net.minecraft.resources.ResourceLocation;

import javax.annotation.Nullable;

public class QuestLinkData {

    public final String questLineId;      // The ID of the quest line to start
    public final ResourceLocation entityId; // The entity type, e.g., "minecraft:wolf"
    @Nullable
    public final String name;             // Optional: specific entity name

    public QuestLinkData(String questLineId, ResourceLocation entityId, @Nullable String name) {
        this.questLineId = questLineId;
        this.entityId = entityId;
        this.name = name;
    }

    /** Convert a JSON element into this data object */
    public static QuestLinkData fromJson(JsonElement element) {
        JsonObject json = element.getAsJsonObject();
        String questLineId = json.get("quest_line_id").getAsString();
        ResourceLocation entityId = new ResourceLocation(json.get("entity_id").getAsString());
        String name = json.has("name") ? json.get("name").getAsString() : null;
        return new QuestLinkData(questLineId, entityId, name);
    }

}
